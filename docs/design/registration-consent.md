...
### Dev/CI seed users bypass the required action

Keycloak's `default_action = true` applies to ALL users created through
the Admin API — not just users who self-registered through the browser
form. This means `dev-seed.sh` users and CI smoke-test users (created via
`POST /admin/realms/minerals/users`) would also get the action stamped,
causing the direct-grant (ROPC) password flow to return 400 "Account is
not fully set up".

**Fix**: after creating each seed/CI user, the provisioning code
explicitly clears `requiredActions` to `[]` via
`PUT /admin/realms/{realm}/users/{id}`. This is idempotent (no-op on
warm re-runs where the user already has no pending actions) and applies
to:

- `terraform/keycloak/dev-seed.sh` — all seeded fixture users
- `.github/workflows/pr.yml` — the default-shaped smoke-test user
  (`smoke-default@localhost`)

Human self-registrants through the Keycloak browser form are unaffected:
their accounts are not created via the Admin API, and the required action
remains pending for them until they accept during their first login flow.
