...
| `INCIDENT_REGISTER_DATABASE_URL` | env | _(unset)_ | no | Postgres DSN for the **separate** database backing the Law 25 confidentiality-incident register (mi-2p6i). MUST differ from `DATABASE_URL` — pointing both at the same database is a hard boot error, because the register must be physically isolated from app data and the GDPR right-to-erasure flow (mi-nwg5). When unset the register store is not wired: its `/api/v1/admin/incident-register/*` endpoints are not registered and the admin console reports the section as `planned` — so a single-database `docker compose up` keeps working. When set, the backend opens a second pool and bootstraps the register's own schema there via `CREATE TABLE IF NOT EXISTS` (the schema is owned by `internal/incidentregister`, deliberately NOT a `migrations/` file). | `internal/config/config.go` |
...
| `WEB_SERVE_MODE` | env | `embedded` | no | `embedded` / `disabled` (mi-zomq). `embedded` — the backend serves the SPA from its embedded `dist/` via the `/` catch-all (v1 single-replica behavior; the root cause of multi-replica asset skew). `disabled` — the backend serves API/docs/health ONLY and skips the `/` catch-all, so the SPA is served from a single shared source (MinIO `minerals-web` bucket / CDN) at the static/ingress layer. Flip the prod overlay to `disabled` only after the decoupled bundle + ingress route are live. See `docs/design/spa-decoupling.md`. | `internal/config/config.go` |
...
| `KEYCLOAK_ADMIN_BASE_URL` | env | _(unset)_ | no | Keycloak server root (e.g. `https://kc.example.com`, **no** `/realms/...` suffix) for the GDPR account-erasure admin client (mi-nwg5). Together with `KEYCLOAK_REALM`, `KEYCLOAK_ADMIN_CLIENT_ID`, and `KEYCLOAK_ADMIN_CLIENT_SECRET` it enables deleting the IdP user behind a deleted app account. When any of the four is empty, account deletion still succeeds but leaves the Keycloak user in place (the app row + sessions are gone). | `internal/config/config.go` |
| `KEYCLOAK_REALM` | env | `minerals` | no | Realm the app's users live in; used to build the admin-API path for account erasure (mi-nwg5). | `internal/config/config.go` |
| `KEYCLOAK_ADMIN_CLIENT_ID` | env | _(unset)_ | no | Client ID of the Keycloak service account used for account-erasure IdP-user deletion (mi-nwg5). The service account must hold the realm-management `manage-users` role. | `internal/config/config.go` |
| `KEYCLOAK_ADMIN_CLIENT_SECRET` | env | _(unset)_ | no | Client secret for `KEYCLOAK_ADMIN_CLIENT_ID` (mi-nwg5). Authenticates the admin client-credentials grant. Treat as a secret — never log; provision via SealedSecret in prod. | `internal/config/config.go` |
